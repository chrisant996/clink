...
features can also be found in GNU's [Readline http://tinyurl.com/oum26rp]
and [History http://tinyurl.com/p92oq5d]
libraries' manuals.
...
The easiest way to configure Clink is to use Clink's //set// command line option.
This can list, query, and set Clink's settings. Run //clink set --help// from a
...
available to configure Readline in Readline's [manual http://tinyurl.com/oum26rp].
...
Clink looks for Lua scripts in the folders as described in the **Configuring Clink**
section. By default //Ctrl-Q// is mapped to reload all Lua scripts which
...
using the variable //rl_state.line_buffer//). If no further match generators need
...
dependent on the previous words (//git merge ...// compared to //git log ...//
...
they are displayed. See **Display Filtering** section for more info.
...
where //text// is the portion of the line buffer that is to be completed,
//first// and //last// are the start and end indices into the line buffer for
//text//.
...
This runs the parser for the table of words //parts//. It returns a table of
...
argument at //index// rather than terminating the parse.
...
is exposed via //doskey// or progromatically via the AddConsoleAlias() function.
...
Retrieves the Clink setting //name//, returning it as a string.
See **Settings** for more information on the available settings.
...
[Readline manual http://tinyurl.com/oum26rp].
...
Globs files using //pattern// and adds results as matches. If //full_path// is
//true// then the path from //pattern// is prefixed to the results (otherwise
only the file names are included). The last argument //find_func// is the
function to use to do the globbing. If it's unspecified (or nil) Clink falls back
to //clink.find_files//.

==== clink.match_words(text, words) ====

Calls clink.is_match() on each word in the table //words// and adds matches to
Clink that match the needle //text//.
...
Splits the string //str// into pieces separated by //sep//, returning a table of
the pieces.
...
Clink exposes a small amount of state from Readline in the global //rl_state//
...
Due to differences between Windows and Linux, escape codes for keys like
PageUp/Down and the arrow keys are different in Clink. Escape codes take the
format //\e`?// where '?' is one of the characters from the following table;
...
Here is an example line from a clink_inputrc file that binds Shift-End to the
Readline function //transpose-word// function;
...
% vim: textwidth=81 expandtab fo+=a fo-=q nosmartindent
