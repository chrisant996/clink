...
<a name="match_translate_slashes"></a>`match.translate_slashes` | `system` | File and directory completions can be translated to use consistent slashes.  The default is `system` to use the appropriate path separator for the OS host (backslashes on Windows).  Use `slash` to use forward slashes, or `backslash` to use backslashes.  Use `auto` to use whichever kind of slash occurs first in the word being completed, or otherwise use the appropriate path separator for the OS host (backslashes on Windows).  Use `off` to turn off translating slashes from custom match generators.
