...
<a name="exec_cwd_first"></a>`exec.cwd_first` | False | When enabled, matches from the current directory are listed first, followed by the other matches (such as doskey aliases, cmd commands, files found along the PATH environment variable, etc).
...
<a name="suggestionlist_max_width"></a>`suggestionlist.max_width` | `100` | The maximum width in columns for the suggestion list.  When this is 0 or negative, the only limit is the terminal width.  When this is 72 or greater, the suggestion list uses up to this value or the terminal width, whichever is smaller.  Otherwise, it uses up to 72 or the terminal width, whichever is smaller.
...
<a name="configexpandtilde"></a>`expand-tilde` | If set to "on", tilde expansion is performed when Readline attempts word completion. The default is "on" in Clink.
...
Command | Key | Description
-|:-:|-
<a name="rlcmd-digit-argument"></a>`digit-argument` | <kbd>Alt</kbd>-<kbd><em>digit</em></kbd> or <kbd>Alt</kbd>-<kbd>-</kbd> | Add this <em>digit</em> to the argument already accumulating, or start a new argument. <kbd>Alt</kbd>-<kbd>-</kbd> starts a negative argument.<br/>**Note:** to enter a negative number that doesn't start with the digit `1`, type the digits without holding <kbd>Alt</kbd>  For example, to enter -2 you can type <kbd>Alt</kbd>-<kbd>-</kbd> and then <kbd>2</kbd>.
...
<a name="rlcmd-clink-cancel-suggestion-list"></a>`clink-cancel-suggestion-list` | | Turn off the [suggestion list](#suggestion-list).
...
<a name="rlcmd-clink-diagnostics-output"></a>`clink-diagnostics-output` | <kbd>Ctrl</kbd>-<kbd>x</kbd> <kbd>Ctrl</kbd>-<kbd>Shift</kbd>-<kbd>z</kbd> | Write internal diagnostic information to a file.
...
<a name="rlcmd-clink-show-suggestion-list"></a>`clink-show-suggestion-list` | | Turn on the [suggestion list](#suggestion-list).
...
<a name="rlcmd-win-copy-up-to-char"></a>`win-copy-up-to-char` | | Enter a character and copy up to it from the previous command (mimics Windows console <kbd>F2</kbd>).
