...
static int32_t s_fe0f_width = 0; // Must be 0 normally, for finding grapheme boundaries.
...
static int32_t resolve_ambiguous_wcwidth(char32_t ucs);
...
static int32_t bisearch(char32_t ucs, const struct interval *table, int32_t max) {
  int32_t min = 0;
  int32_t mid;
...
static int32_t mk_wcwidth(char32_t ucs)
{
...
static int32_t mk_wcwidth_ucs2(char32_t ucs)
{
...
// Use __wcswidth() instead:  it handles fully qualified color emoji, which
// requires sometimes looking at TWO codepoints to determine the width.
#if 0
static int32_t mk_wcswidth(const char32_t *pwcs, size_t n)
{
  int32_t w, width = 0;
 ...
 */
static int32_t mk_wcwidth_cjk(char32_t ucs)
{
...
static int32_t mk_wcwidth_cjk_ucs2(char32_t ucs)
{
  /* binary search in table of ambiguous width chars in CJK codepages */
  if (bisearch(ucs, ambiguous, _countof(ambiguous) - 1))
    return resolve_ambiguous_wcwidth(ucs);
...
#if 0
static int32_t mk_wcswidth_cjk(const char32_t *pwcs, size_t n)
{
  int32_t w, width = 0;
...
wcwidth_func_t* wcwidth = mk_wcwidth;

#if 0
typedef int32_t (*wcswidth_func_t)(const char32_t*, size_t);
wcswidth_func_t* wcswidth = mk_wcswidth;
#endif
  ...
  if (s_hdc && s_hfont && s_cell)
  {
    if (ucs < 0x10000)
    {
      ...
      if (GetCharABCWidthsW(s_hdc, ucs, ucs, &abc))
      {
        ...
        int32_t width = (abc.abcA + abc.abcB + abc.abcC);
        if (width > 0)
        {
    ...
    *p = '\0';
    ...
    SIZE size;
    if (GetTextExtentPoint32W(s_hdc, tmp, int32_t(p - tmp), &size))
    {
      LOG("Extent:  0x%X => cx %d, cell %d", uint32_t(ucs), size.cx, s_cell);
...
#ifdef _WIN32
int32_t is_CJK_codepage(UINT cp)
{
