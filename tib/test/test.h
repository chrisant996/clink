...
// Derived from clink/clatch.*, portions Copyright (c) 2016 Martin Ridgers
...
inline void fail(const char* expr, const char* file, int32_t line)
{
    section* failed_section = section::get_outer_store();
...
void fail(const char* expr, const char* file, int32_t line, callback&& cb)
{
    puts("\n");
    printf(colors::get_error());
    cb();
...
#define TEST_IDENT__(d, b) _test_##d##_##b
#define TEST_IDENT_(d, b)  TEST_IDENT__(d, b)
#define TEST_IDENT(d)      TEST_IDENT_(d, __LINE__)
...
#define REQUIRE(expr, ...)\
    do {\
        auto* _test_s = test::section::get_outer_store();\
        for (; _test_s->m_parent != nullptr; _test_s = _test_s->m_parent);\
        ++_test_s->m_assert_count;\
        \
        if (!(expr))\
